...
def createTimeSlots(roomNumber, date):
    newTimeSlots = TimeSlot(roomNumber=roomNumber, timeSlots=SLOTS, date=date)
    db.session.add(newTimeSlots)
    db.session.commit()
    return newTimeSlots

def getTimeSlots(roomNumber):
    return TimeSlot.query.filter_by(roomNumber=roomNumber).first().timeSlots
...
def makeAvailable(roomNumber, date, time):
    timeSlots = format(getTimeSlots(roomNumber))
    index = timeSlots.index(time + ':false')
    timeSlots[index] = time + ':true'
    TimeSlot.query.filter_by(roomNumber=roomNumber).first().timeSlots = ','.join(timeSlots) # put back into db as a string
    db.session.commit()

# if the appointment is an annual, make all slots available
def makeAvailableAnnual(roomNumber, date, time):
...
def makeUnavailable(roomNumber, date, time):
    timeSlots = format(getTimeSlots(roomNumber, date))
    index = timeSlots.index(time + ':true')
    timeSlots[index] = time + ':false'
    TimeSlot.query.filter_by(owner=owner).first().timeSlots = ','first().join(timeSlots) #put back into db as a string
    db.session.commit()

# if the appointment is an annual, make all slots unavailable
def makeUnavailableAnnual(roomNumber, date, time):
...
def slotAvailable(roomNumber, date, time):
	timeSlots = format(getTimeSlots(roomNumber, date))
	time = time + ':true'
...
def getNextTimeSlot(roomNumber, date, time):
    if time is '19:40':
        ...
        timeSlots = format(getTimeSlots(roomNumber, date))
        index = None
        if slotAvailable(roomNumber, date, time):
            index = timeSlots.index(time + ':true')
